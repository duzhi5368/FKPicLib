...
//-------------------------------------------------------------------------
CFKImageFKP::CFKImageFKP()
	: CFKImage( eFormats_FKP )
{
#if USE_FK_EXIF
	m_pExif = NULL;
...
//-------------------------------------------------------------------------
bool CFKImageFKP::Decode( IFKFile* p_hFile )
{
	bool bIsExif = false;
#if USE_FK_EXIF
	...
	{
		p_hFile->Seek( -18, SEEK_END );
		char szLen[8];
		memset( szLen, 0, 8 );
		int nLoad = p_hFile->Read( szLen, 8, 1 );
		if( nLoad <= 0 )
			return false;
		int nLen = atoi( szLen );
		if( nLen <= 0 )
		...
		BYTE* szSrcBuffer = new BYTE[nLen];
		memset( szSrcBuffer, 0, nLen );
		p_hFile->Seek( -18 - nLen, SEEK_END );
		nLoad = p_hFile->Read( szSrcBuffer, nLen, 1 );
		if( nLoad <= 0 )
			return false;

		uLongf lDstLen = GetWidth() * GetHeight();
		BYTE* szDstBuffer = new BYTE[lDstLen];
		memset( szDstBuffer, 0, lDstLen );
		int nRet = uncompress( szDstBuffer, &lDstLen, szSrcBuffer, nLen );
		if( nRet != Z_OK )
		{
			strcpy( m_tagInfo.m_szLastError, "Zlib uncompress failed" );
			return false;
		}
		...
		for( unsigned int i = 0; i < GetWidth(); ++i )
		{
			...
			{
				SetAlpha( i, j, szDstBuffer[i + j * GetWidth() ] );
			}
		}
...
//-------------------------------------------------------------------------
bool CFKImageFKP::Decode( FILE* p_hFile )
{
	CFKIOFile file( p_hFile );
	return Decode( &file );
}
//-------------------------------------------------------------------------
bool CFKImageFKP::Encode( IFKFile* p_hFile )
{
	...
	if( IsAlphaValid() )
	{
		unsigned int unSize = GetWidth() * GetHeight();
		BYTE* pAlphaBuf = new BYTE[unSize];
		memset( pAlphaBuf, 0, unSize );
		BYTE* pSrcAlpha = GetAlphaPointer();
		memcpy( pAlphaBuf, pSrcAlpha, unSize );

		unsigned int unDstSize = compressBound( unSize );
		BYTE* pDstAlphaBuf = new BYTE[unDstSize];
		memset( pDstAlphaBuf, 0, unDstSize );
		uLongf lDstLen;
		int nError = -1;
		if( GetZipQuality() == 0 )
		{
			nError = compress2( pDstAlphaBuf, &lDstLen, pAlphaBuf, unSize, Z_DEFAULT_COMPRESSION );
		}
		else
		{
			nError = compress2( pDstAlphaBuf, &lDstLen, pAlphaBuf, unSize, GetZipQuality() );
		}
		if( nError != Z_OK )
		{
			strcpy( m_tagInfo.m_szLastError, "Zlib compress failed" );
			return false;
		}

		p_hFile->Write( pDstAlphaBuf, lDstLen, 1 );
		char szLen[8];
		memset( szLen, 0, 8 );
		sprintf( szLen, "%d", lDstLen );
		p_hFile->Write( szLen, 8, 1 );
...
//-------------------------------------------------------------------------
bool CFKImageFKP::Encode( FILE* p_hFile )
{
	CFKIOFile file( p_hFile );
	return Encode( &file );
}
//-------------------------------------------------------------------------
void CFKImageFKP::JpegErrorExit( j_common_ptr p_tagInfo )
{
...
#if USE_FK_EXIF
//-------------------------------------------------------------------------
bool CFKImageFKP::DecodeExif( IFKFile* p_hFile )
{
...
//-------------------------------------------------------------------------
bool CFKImageFKP::DecodeExif( FILE* p_hFile )
{
